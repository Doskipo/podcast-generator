...
def _build_prompts(profile: Profile, articles: list[Article], sampled_moods: dict[str, str]) -> tuple[str, str]:
    ...
    system_prompt = (
        ...
        "Nothing below should read as quoting the persona verbatim. A host's home turf is "
        "part of the same thing: it describes character, expertise, and taste, not a "
        "checklist of subjects that must appear. Let it shape *how* a host engages with a "
        "story — their angle, their reaction, an analogy they'd reach for — never force a "
        "pet subject into a story where it doesn't genuinely belong.\n\n"
        ...
        "sure a link is real; never invent a connection just to justify one. Sharing only a "
        "broad topic or category is not a genuine connection — a set of separate results or "
        "announcements (e.g. results from different races in the same season, or several "
        "unrelated product launches) must use clean_transition between them even though "
        "they're topically adjacent. "
