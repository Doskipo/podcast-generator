...
STAGES = ["fetch", "rank", "outline", "script", "critique", "perform", "quality", "tts", "stitch"]
...
def run_from_articles(articles_path: str, until: str | None = None) -> str:
    ...
    critique -> perform -> quality -> tts -> stitch."""
...
def run_from_outline(outline_path: str, until: str | None = None) -> str:
    ...
    quality -> tts -> stitch. Also needs the episode's ranked.json (same
    directory) for the selected articles' full text."""
...
def run_from_script(script_path: str, until: str | None = None) -> str:
    """Re-run from a persisted script.json: critique -> perform -> quality -> tts ->
...
def run_from_critique(critique_path: str, until: str | None = None) -> str:
    """Re-run from a persisted critique.json: perform -> quality -> tts -> stitch. Also
...
def run_from_performance(performance_path: str, until: str | None = None) -> str:
    """Re-run from a persisted performance.json: quality -> tts -> stitch."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--from-articles",
        default=None,
        help="path to a persisted articles.json; skips fetch, re-runs rank -> outline -> script -> critique -> perform -> quality -> tts -> stitch",
    )
    parser.add_argument(
        "--from-ranked",
        default=None,
        help="path to a persisted ranked.json; skips fetch/rank, re-runs outline -> script -> critique -> perform -> quality -> tts -> stitch",
    )
    parser.add_argument(
        "--from-outline",
        default=None,
        help="path to a persisted outline.json; skips through outline, re-runs script -> critique -> perform -> quality -> tts -> stitch",
    )
    parser.add_argument(
        "--from-script",
        default=None,
        help="path to a persisted script.json; skips through script, re-runs critique -> perform -> quality -> tts -> stitch",
    )
    parser.add_argument(
        "--from-critique",
        default=None,
        help="path to a persisted critique.json; skips through critique, re-runs perform -> quality -> tts -> stitch",
    )
    parser.add_argument(
        "--from-performance",
        default=None,
        help="path to a persisted performance.json; skips through perform, re-runs quality -> tts -> stitch",
    )
    parser.add_argument(
        "--until",
        choices=STAGES,
